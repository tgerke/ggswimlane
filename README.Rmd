...
ggswimlane is a ggplot2 extension for swimlane (swimmer) plots, a common way
to show patient trajectories through a clinical trial: one bar per subject,
with events, status, and baseline characteristics marked along each lane.
Legends come for free, and the default palette and theme are ready for
reports without further styling.
...
```{r example, message=FALSE, fig.width=9, fig.height=6, fig.alt="Swimlane plot of 20 subjects colored by cohort, with shaped points marking end-of-study status, partial responses, prior therapy flags in the left margin, and arrows for subjects still on study"}
library(dplyr)
library(ggplot2)
library(ggswimlane)
...
The [gallery vignette](https://tgerke.github.io/ggswimlane/articles/gallery.html)
